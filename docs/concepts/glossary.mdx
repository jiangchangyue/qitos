...
A ready-made Agent + Critic pair that implements a well-known agentic reasoning pattern. QitOS ships with Self-Refine and Reflexion templates in `qitos.recipes`. Each template packages a specialized state, critic, and agent.
