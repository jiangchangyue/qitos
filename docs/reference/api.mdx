...
`get_all_specs()` returns OpenAI-compatible function specs for native tool delivery.
Unbounded `**kwargs` parameters are omitted from the exported schema, and `Any`
parameters are rendered without the invalid JSON Schema type `any`.
